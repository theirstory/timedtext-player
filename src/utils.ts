...
export function getCaptions(segment: Clip): string {
  ...
  (captions3 as any).forEach((tt: TimedText[], i: number) => {
    ...
    const text = tt.map((t) => `<${formatSeconds(t.marked_range.start_time)}>`
      + t.metadata.ruby
...
export function dom2otio(sections: NodeListOf<HTMLElement> | undefined): {track: Track, duration: number} | undefined {
  if (!sections) return;
